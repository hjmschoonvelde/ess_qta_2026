# QTA Lab 10 Answers: LLM-Assisted Coding
...
![](Lab_Session_QTA_10_Answers_files/figure-commonmark/plot_credit_blame_by_leader-1.png)
...
``` r
binary_blame_instructions <- paste(
  "Decide whether the speech contains blame attribution.",
  "Code TRUE when the speaker attributes a negative outcome, failure, harm, or public problem to an actor.",
  "Code FALSE when the speech is descriptive, procedural, only claims credit, or does not clearly attribute responsibility for a negative outcome.",
  "Use only the speech text. Do not infer from party identity, leader identity, or outside knowledge.",
  "Provide brief evidence for the decision."
)

binary_blame_instructions
```

    [1] "Decide whether the speech contains blame attribution. Code TRUE when the speaker attributes a negative outcome, failure, harm, or public problem to an actor. Code FALSE when the speech is descriptive, procedural, only claims credit, or does not clearly attribute responsibility for a negative outcome. Use only the speech text. Do not infer from party identity, leader identity, or outside knowledge. Provide brief evidence for the decision."
...
``` r
binary_blame_schema <- ellmer::type_object(
  blame_attribution = ellmer::type_boolean(
    "TRUE if the speech contains blame attribution; FALSE otherwise."
  ),
  confidence = ellmer::type_number(
    "Confidence from 0 to 1."
  ),
  evidence = ellmer::type_string(
    "Short quote or close paraphrase supporting the decision."
  )
)

binary_blame_codebook <- qlm_codebook(
  name = "Binary HoC leader blame attribution",
  role = "You are a careful qualitative coder of UK parliamentary speeches.",
  instructions = binary_blame_instructions,
  schema = binary_blame_schema
)

binary_blame_codebook
```

    quallmer codebook: Binary HoC leader blame attribution 
      Input type:   text
      Role:         You are a careful qualitative coder of UK parliamentary spee...
      Instructions: Decide whether the speech contains blame attribution. Code T...
      Output schema:ellmer::TypeObject
...
``` r
binary_coded_leaders <- qlm_code(
  leader_texts[1:5],
  binary_blame_codebook,
  model = "ollama/llama3.2",
  params = ellmer::params(temperature = 0),
  name = "binary_blame_llama32_temperature0"
)

binary_coded_leaders
```
...
``` r
mock_human_binary <- ollama_coded_leaders |>
  slice_head(n = 8) |>
  transmute(
    .id,
    blame_attribution = case_when(
      row_number() %in% c(2, 6) ~ FALSE,
      TRUE ~ blame_attribution
    )
  )
...
``` r
human_binary <- qlm_humancoded(
  mock_human_binary,
  name = "expert_binary_blame_coder",
  codebook = binary_blame_codebook,
  texts = leader_texts,
...
```
    ...
    # A tibble: 2 × 5
      party        leader            n credit_rate blame_rate
      <fct>        <chr>         <int>       <dbl>      <dbl>
    1 Conservative Boris Johnson     4        0          0.5 
    2 Labour       Ed Miliband       4        0.25       0.75
...
ollama_coded_leaders |>
  ...
  left_join(
    leader_llm |>
      select(speech_id, party, leader, agenda, text_for_coding),
    by = c(".id" = "speech_id")
  ) |>
  select(.id, party, leader, agenda, evidence, text_for_coding) |>
  slice_head(n = 3)
```

    # A tibble: 2 × 6
      .id         party        leader        agenda      evidence    text_for_coding
      <chr>       <fct>        <chr>         <chr>       <chr>       <chr>          
    1 leader_5492 Conservative Boris Johnson Covid-19    The abilit… I thank my hon…
    2 leader_5518 Conservative Boris Johnson Engagements We will co… I know that th…

``` r
# To judge whether "neither" is correct, inspect whether the speech claims
# responsibility for a positive outcome or attributes a negative outcome to an
# actor. If those elements appear but the Ollama output misses them, the codebook
# or model needs revision.
```
...
``` r
# A methods section should report:
# - the leader corpus and sampling strategy;
# - the concept and full codebook;
# - the structured output schema;
# - model/provider/version and whether it was local or API-based;
# - ellmer parameters such as temperature;
# - input truncation or chunking rules;
# - whether metadata were included in the prompt;
# - validation sample design and human coding procedure;
# - validation metrics by label, party, and leader where possible;
# - examples of disagreements and how the codebook was revised.
```
